...
exports.getSupplier = async (req, res) => {
  ...
  try {
    const query = await Profile.find({ listedMaterials: materialId });
    const suppliers = [];
    query.forEach((q) => suppliers.push(q.walletAddress));
    return res.status(200).json({
      message: "Successful",
      suppliers: suppliers,
      timestamp: Date.now(),
      path: `/profiles/${materialId}/suppliers`,
      method: "GET",
    });
  } catch (err) {
    return res.status(400).json({
...
exports.getManufactures = async (req, res) => {
  ...
  try {
    const query = await Profile.find({ listedProducts: productId });
    const manufacturers = [];
    query.forEach((q) => manufacturers.push(q.walletAddress));
    return res.status(200).json({
      message: "Successful",
      manufacturers: manufacturers,
      timestamp: Date.now(),
      path: `/profiles/${productId}/manufacturers`,
      method: "GET",
    });
  } catch (err) {
    return res.status(400).json({
